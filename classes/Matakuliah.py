...
class Matakuliah:
    ...
    def update(self, id):
        ...
        sql="UPDATE matakuliah SET kodemk=%s, namamk=%s, sks=%s WHERE idmk=%s"
    ...
    def updateByKODEMK(self, kodemk):
        self.conn = mydb()
        val = (self.__kodemk,self.__namamk,self.__sks, kodemk)
        sql="UPDATE matakuliah SET kodemk=%s, namamk=%s, sks=%s WHERE kodemk=%s"
        self.affected = self.conn.update(sql, val)
        self.conn.disconnect
        return self.affected
